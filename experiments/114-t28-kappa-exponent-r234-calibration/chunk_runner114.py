"""Checkpoint/resume runner for exp-114's own r=234/cpl=25 leg (3 scenes:
empty, hollow, peccored) -- Panel Iteration 91. Extends
experiments/113-.../chunk_runner113.py's own checkpoint/resume idiom
...
A/B-tested finding, reused without re-testing) with the same
time-BUDGETED sub-chunk loop (--budget-s), checkpointed every SUBSTEP.

Also hosts the R31 same-session control mode (--control), REUSING
run114.py's own r31_control_ratio/combine_control_readings (imported,
unmodified, themselves re-exports of run113.py's own r-independent
functions) -- re-times `control_steps` of the ALREADY-COMPLETED
r=156/cpl=25 empty+hollow+peccored scenes, fresh, cold-build, no
checkpoint reuse, at the START of this session, before the cost gate is
evaluated for real. See run114.py::cost_gate_check_r31_r234.
...
assert hasattr(R, "geom_fixedabs_cpl") and hasattr(R, "cost_gate_check_r31_r234"), (
    "R29: R must be exp-114's own run114.py")

SCRATCH = os.path.join(
    "/tmp/claude-0/-home-user-photon-lab/0158c3db-3adc-567e-82f3-fc3e8d29a0e4/scratchpad",
    "exp114")
...
SHORT_CONTROL_STEPS = 1000      # per scene, matching exp-113's own convention
SUSTAINED_CONTROL_STEPS = 3334  # per scene, matching exp-113's own Fix 4 convention
...
def _time_control_blend(control_steps_per_scene, scenes=CONTROL_SCENES):
    ...
    steps, summed. Byte-for-byte the same recipe as exp-113's own
    chunk_runner113.py::_time_control_blend, retargeted at run114's own
    r31_control_ratio (itself a re-export of run113's own function)."""
...
def run_control():
    """R31 same-session control, identical in structure to exp-113's own
    run_control() -- a short and a sustained 3-scene-blend reading,
    combined by R.combine_control_readings() (gates on the LOWER, more
    conservative speed_ratio of the two). Written to its own
    control-tagged path under THIS experiment's own scratch directory."""
...
def check_cost_gate_for_r234(cpl):
    ...
    with open(control_path) as f:
        control = json.load(f)
    pilot_empty = R.HISTORICAL_R156_CPL25_TOTAL_S / 3.0  # avg per-scene, historical (r=156/cpl=25)
    pilot_total = R.HISTORICAL_R156_CPL25_TOTAL_S
    gate = R.cost_gate_check_r31_r234(pilot_empty, pilot_total, control)
    with open(os.path.join(SCRATCH, f"r234_cpl{cpl}_costgate.json"), "w") as f:
        json.dump(gate, f, indent=2)
    print(json.dumps(gate, indent=2))
    if not gate["proceed_to_r234"]:
        raise RuntimeError(f"R27/R28/R31 cost gate REFUSED r=234 (cpl={cpl}): {gate}")
    return gate
...
if __name__ == "__main__":
    if sys.argv[1] == "--control":
        ...
        check_cost_gate_for_r234(cpl_arg)
    else:
        ...
        if r_arg == 234:
            check_cost_gate_for_r234(cpl_arg)   # genuinely upstream (R28), re-verified every call
