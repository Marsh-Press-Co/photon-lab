"""exp-024 -- Instrument Margin + Estimator Adjudication: measurement harness.
===============================================================================
Panel Iteration 2 (lead PHOTONICS; synthesis: Director, post Red Team's
proceed-with-mandatory-fixes). Reruns exp-020's four articles (empty /
absorber / PEC / dilute sponge) on a corrected instrument: MARGIN_MULT=3.5
(not the 2x/3x either Phase-1 or the EM flip proposed) pushes the worst-case
margin/fringe-zone ratio to 3.5-4.5 at every committed wavelength -- well
clear of the ratio~1 threshold-collapse regime Red Team's backtest showed
made exp-020's own extrapolation untrustworthy (see NOTES.md Phase 3).

Same structure as exp-020: 124 runs (4 articles x 9 angles x 3 lambda + 16
convergence). Geometry (NY=1584, OBJ=(170,792), BOX, SRC_Y) comes entirely
from design_geometry.py -- single source of truth, re-run it if any constant
moves. Predictions committed in NOTES.md BEFORE this file's first run (house
discipline).
"""
...
V_PHOT = {450: 0.038, 600: 0.631, 750: 0.00012}    # CIE 1924 V(lambda)
V_SCOT = {450: 0.455, 600: 0.033, 750: 1e-6}       # CIE 1951 V'(lambda)

DELTA_C_GATE = 0.001        # Director's synthesis, Phase 3: tightened from
                             # PHOTONICS' 0.003 proposal per VISION's flip
                             # + Red Team #4 (0.003 doesn't reach decidability
                             # against the pinned OFF-lab sigma(I) target
                             # C=-0.005). Hard gate at every lambda, not just
                             # 600/750 -- the whole point of the wider margin.
...
def run_group(args):
    """One (lambda, theta) group: empty first, then the requested articles
    measured against that empty. Returns per-article profiles + ledger rows."""
    ...
    net_e = sc._face_flux(ph_e["ez"], ph_e["hx"], ph_e["hy"], dg.BOX)
    ...
    for art in articles:
        ...
        profs = {d: amb.observer_profile(ph, dg.OBJ[0] - dg.R_OUT - d,
                                         dg.ABSORB, dg.NY - dg.ABSORB)
                 for d in dg.SENS_PLANES}
        wd = sc.widths(cap, cap_e, dg.BOX, REF)
...
def main():
    ...
    for th in dg.N17_EXTRA:                        # convergence set @600nm
        groups.append((600, 20, float(th), ["empty", "pec"]))

    n_runs = sum(len(g[3]) for g in groups)
    print(f"exp-024: {len(groups)} groups, {n_runs} runs, "
          f"{os.cpu_count()} cpus, NY={dg.NY}, MARGIN_MULT={dg.MARGIN_MULT}")
    t0 = time.time()
    results = {}
    with ProcessPoolExecutor(max_workers=4) as ex:
        for lam_nm, theta, out, dt in ex.map(run_group, groups):
            ...
            print(f"  [{done:2d}/{len(groups)}] lambda={lam_nm} theta={theta:+05.1f} "
                  f"({dt:5.1f} s)", flush=True)

    # ---------------- assemble contrasts (per article, lambda, weighting) --
    ...
    # ---------------- harness gates (P1b, P6, decision floor) --------------
    gates = {}
    for lam_nm, _ in LAMBDAS:
        for th in (40.0, -40.0):
    ...
    out = {
        "meta": {"geometry": {k: getattr(dg, k) for k in
                 ...
                 "note": ("sigma values are per-(theta,lambda) normalized by "
                          ...
                          "throughout (lab/ambient.py). Raw C is the ONLY "
                          "reported currency (Director's Phase-3 ruling): no "
                          "floor-correction estimator is applied."),
                 "elapsed_s": time.time() - t0},
        "contrasts": table, "luminance_weighted": lum,
        "plane_sensitivity": planes, "convergence": conv,
        "decision_floors": floors, "delta_c_pass": delta_c_pass, "gates": gates,
    }
    ...
    print(f"\ndecision floors delta_C (gate <= {DELTA_C_GATE}):")
    for k, v in floors.items():
        print(f"  {k}: {v:.6f}  {'PASS' if v <= DELTA_C_GATE else 'FAIL'}")
    print("\nWeber contrast C (plane 15, equal weights, RAW -- no correction):")
    for art in ARTICLES:
    ...
    print(f"delta_C all-lambda pass ({DELTA_C_GATE}): {all(delta_c_pass.values())}")
    ...
    ax.set_title("exp-024 -- margin-fixed ambient instrument, 600 nm",
